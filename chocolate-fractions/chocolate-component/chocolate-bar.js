...
(function () {
  ...
  function ChocolateBar(props) {
    const {
      ...
      greyed = false,           /* dark grey overlay on the unselected whole (storyboard de-highlight) */
      ...
      pulseShaded = false,      /* put focus outline on ALL shaded segments (count-pulse) */
      ...
      segmentSlide = null,      /* { index?, indices?, dx, dy, duration?, misalign? } slide one or more segments */
      ...
      onTapBar = null,
      extraClass = '',
      shadeFill = null,   /* override shaded segment fill color (hex) */
      guideOverlay = null, /* SVG/node rendered absolutely over the bar SVG */
    } = props;
    ...
    /* Build SVG segments */
    const slideIndices = segmentSlide
      ? (Array.isArray(segmentSlide.indices) ? new Set(segmentSlide.indices) : new Set([segmentSlide.index]))
      : null;
    const segmentsEls = [];
    for (let i = 0; i < segments; i++) {
      ...
      const isPulseSeg = pulseSegment === i || (pulseShaded && isShaded);
      const isSlidingSeg = slideIndices && slideIndices.has(i);
      const segClass = [
        'chocolate-segment',
      ...
      if (isShaded) {
        if (shadeFill) {
          /* Replace orange base with user color — solid radial gradient, no orange bleed */
          const segShadeId = gradId + '-shade-' + i;
          /* Lighten center slightly for a natural sheen */
          segChildren.unshift(
            h('defs', null,
              h('radialGradient', { id: segShadeId, cx: '50%', cy: '35%', r: '65%', fx: '50%', fy: '30%' },
                h('stop', { offset: '0%', 'stop-color': shadeFill, 'stop-opacity': '0.75' }),
                h('stop', { offset: '50%', 'stop-color': shadeFill, 'stop-opacity': '1' }),
                h('stop', { offset: '100%', 'stop-color': shadeFill, 'stop-opacity': '1' })
              )
            )
          );
          /* Replace the orange base rect fill with the user color gradient */
          segChildren[1] = h('rect', {
            x: 0, y: 0, width: segWidth, height: BAR_HEIGHT,
            fill: 'url(#' + segShadeId + ')', stroke: 'none',
          });
        } else {
          segChildren.push(
            h('rect', {
              x: 0, y: 0, width: segWidth, height: BAR_HEIGHT,
              fill: 'rgba(100, 40, 0, 0.32)', className: 'chocolate-segment-shade',
            })
          );
          segChildren.push(
            h('rect', {
              x: 0, y: 0, width: segWidth, height: BAR_HEIGHT,
              fill: 'url(#hatch-' + gradId + ')', opacity: 0.32,
            })
          );
        }
      }
    ...
    const defs = h(
      ...
      h(
        'radialGradient',
        { id: gradId, cx: '50%', cy: '40%', r: '65%', fx: '50%', fy: '35%' },
        h('stop', { offset: '0%', 'stop-color': '#FFD080' }),
        h('stop', { offset: '45%', 'stop-color': '#F5A020' }),
        h('stop', { offset: '100%', 'stop-color': '#C96A00' })
      ),
