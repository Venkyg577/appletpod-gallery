...
function createDom(node, ns){
  ...
  if (typeof node.type === 'function') return mountComponent(node.type, node.props, ns);
  ...
  // <foreignObject> hosts HTML content: its descendants must be in the HTML namespace, not SVG.
  const childNs = (node.type === 'foreignObject') ? undefined : (isSvg ? namespace : undefined);
...
function setProps(el, props={}){
  Object.entries(props).forEach(([k,v])=>{
    ...
    // raw HTML injection (e.g. KaTeX output); value is a trusted HTML string
    ...
    // boolean attributes (disabled, checked, …): present when true, absent when false —
    // setAttribute('disabled', false) still disables, so only set/remove via the value
    else if (typeof v === 'boolean') {
      if (v) el.setAttribute(k, ''); else el.removeAttribute(k);
      if (k in el) { try { el[k] = v; } catch (e) {} }
    }
    else if (v != null && k !== 'children') el.setAttribute(k, v);
  });
}

function mountComponent(fn, props, ns){
  const comp = { fn, props, hooks: [], hookIndex: 0, dom: null, container: null, pendingEffects: [], ns: ns };
  const prev = currentComponent; currentComponent = comp;
  const vnode = fn(props);
  currentComponent = prev;
  const dom = createDom(vnode, ns);
...
function rerender(comp){
  ...
  const newDom = createDom(vnode, comp.ns);
