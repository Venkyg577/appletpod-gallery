...
export function ensureThreeLibLoaded() {
  ...
  threeLoadPromise = new Promise((resolve, reject) => {
    const existing = document.querySelector('script[data-three-lib="1"]');
    if (existing) {
...
export function updateAtomStructure(element) {
  ...
  if (atomicNumber > 1) {
    ...
    if (atomicNumber === 11 && window._appNaMode === "cloud") lightIntensity = 5.0;
...
export function animateAtom() {
  ...
  const isPaused = window._appAnimPaused || false;
  const speedMul = (typeof window._appAnimSpeed === "number") ? window._appAnimSpeed : 0.6;
  ...
  const isEdu = isNa && window._appNaMode === "educational";
