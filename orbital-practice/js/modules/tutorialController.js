...
function ensureDriverTutorialAssets() {
    if (!document.getElementById('driver-custom-pt')) {
        const style = document.createElement('style');
        style.id = 'driver-custom-pt';
        style.textContent = `
...
export async function initElementTutorial(force = false) {
    const tutorialKey = 'hasSeenElementTutorialV8';
    ...
    try {
        // Lazy-load driver.js locally (bare specifier) so the library is only
        // pulled into a separate chunk when a tutorial actually runs.
