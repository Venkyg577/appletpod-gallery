const ANIMATION_PAUSED_KEY = "pt_anim_paused";
const ANIMATION_SPEED_KEY = "pt_anim_speed";
...
function initAnimationControls() {
  ...
  if (speedSlider) {
    speedSlider.value = window._appAnimSpeed;
    if (speedLabel) {
      speedLabel.textContent = `${window._appAnimSpeed.toFixed(1)}×`;
    }
  }

  if (playToggle) {
    updatePlayToggleIcon(playToggle, window._appAnimPaused);
  }

  applyAnimationPauseState(window._appAnimPaused);

  if (playToggle) {
    playToggle.addEventListener("click", () => {
      window._appAnimPaused = !window._appAnimPaused;
      localStorage.setItem(ANIMATION_PAUSED_KEY, window._appAnimPaused);
      updatePlayToggleIcon(playToggle, window._appAnimPaused);
      applyAnimationPauseState(window._appAnimPaused);
    });
  }
...
function initPreferencesCard() {
  ...
  if (reduceToggle) {
    const isReduced = localStorage.getItem("pt_reduce_motion") === "true";
    reduceToggle.checked = isReduced;
    
    reduceToggle.addEventListener("change", (e) => {
      localStorage.setItem("pt_reduce_motion", e.target.checked);
  ...
  if (clearBtn) {
    clearBtn.addEventListener("click", () => {
      const msg = "Are you sure you want to clear all local data (units, preferences) and reload?";
