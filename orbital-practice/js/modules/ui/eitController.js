...
const EIT_PROPERTY_CONFIG = [
  { key: "category", label: "Category", type: "category", group: "classification" },
  { key: "block", label: "Block", type: "category", group: "classification" },
  { key: "metalType", label: "Metal Type", type: "category", group: "classification" },
  { key: "state", label: "State", type: "category", group: "classification" },
  {
    key: "meltingPoint",
    label: "Melting Point",
    type: "numeric",
...
/** Display labels for discrete category values shown in the custom legend chips. */
const EIT_PROPERTY_VALUE_LABELS = {
  s: "s Block",
  p: "p Block",
  d: "d Block",
  f: "f Block",
    ...
    eitUI.chips.forEach((chip) => {
      ...
      if (chipConfig?.units && chipConfig.units.length > 1) {
        ...
        if (chip.innerHTML !== newHTML) {
          ...
          chip.setAttribute("title", "Click again to change unit");
        }
      } else {
...
function applyCategoryEIT(config) {
  ...
  if (legendContainer) {
    ...
    Object.entries(scheme.colors).forEach(([val, color]) => {
      ...
      const localizedLabel = EIT_PROPERTY_VALUE_LABELS[val.replace(/\s+/g, '')] || val;
...
function applyNumericEIT(config) {
  ...
  if (!numericValues.length) {
    ...
    updateEITLegend({
      visible: true,
      title: `${getPropertyLabel(config)} Range`,
  ...
  if (!root) {
    ...
    const chipsHTML = `
      <div class="eit-chip-group">
        <div class="eit-chip-group-label">Property</div>
        <div class="eit-chip-list">${buildChipsForGroup("property")}</div>
      </div>
      <div class="eit-chip-group-divider"></div>
      <div class="eit-chip-group">
        <div class="eit-chip-group-label">Classification</div>
    ...
    root.innerHTML = `
      ...
      <div class="eit-mode-group" id="eit-mode-group" role="group" aria-label="Visualization mode">
