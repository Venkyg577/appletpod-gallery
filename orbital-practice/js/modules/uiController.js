...
import { initPracticeForElement, bindStepBar } from "../orbital/practiceModalController.js";
import {
  t,
  onLangChange,
  getLang,
...
// v2 dataset stub — file removed; branches gated by appVersion === 'new' are inert.
...
// ===== Content-flow highlight primitives (period / group / block) =====
// Share the .highlighting/.highlighted contract with highlightCategory above,
// so clearHighlights() reverses all of them.

function highlightByPredicate(container, predicate) {
  container.classList.add("highlighting");
  container.querySelectorAll(".element").forEach((el) => {
    const element = elements.find(
      (e) => e.number === Number(el.dataset.elementNumber),
    );
    el.classList.toggle("highlighted", !!element && predicate(element));
  });
}

// row/column on the f-block series are layout slots in the detached bottom
// rows (row 9/10), not real periods/groups — exclude them from both.
function isDetachedSeries(element) {
  return element.series === "lanthanide" || element.series === "actinide";
}

export function highlightPeriod(container, periodNumber) {
  highlightByPredicate(
    container,
    (element) => !isDetachedSeries(element) && element.row === periodNumber,
  );
}

export function highlightGroup(container, groupNumber) {
  highlightByPredicate(
    container,
    (element) => !isDetachedSeries(element) && element.column === groupNumber,
  );
}

// He sits in group 18 visually but its last electron enters the s-orbital.
export function getBlock(element) {
  const { number, column } = element;
  if (isDetachedSeries(element)) return "f";
  if (number === 2) return "s";
  if (column >= 1 && column <= 2) return "s";
  if (column >= 3 && column <= 12) return "d";
  if (column >= 13 && column <= 18) return "p";
  return null;
}

export function highlightBlock(container, blockLetter) {
  highlightByPredicate(container, (element) => getBlock(element) === blockLetter);
}

// Lanthanides/actinides have no meaningful period/group of their own — they
// are described by their series instead.
export function describeElementPlacement(element) {
  if (isDetachedSeries(element)) {
    const seriesName = element.series === "lanthanide" ? "lanthanides" : "actinides";
    return `${element.name} (${element.symbol}) belongs to the ${seriesName}, the f-block rows shown separately at the bottom of the periodic table.`;
  }
  return `${element.name} (${element.symbol}) belongs to Period ${element.row} and Group ${element.column} of the periodic table.`;
}

export function highlightElementCell(container, elementNumber) {
  const element = elements.find((e) => e.number === elementNumber);
  if (!element) return;
  if (isDetachedSeries(element)) {
    highlightByPredicate(container, (e) => e.series === element.series);
  } else {
    highlightByPredicate(
      container,
      (e) =>
        !isDetachedSeries(e) &&
        (e.row === element.row || e.column === element.column),
    );
  }
  container.querySelectorAll(".element-tap-focus").forEach((el) => {
    el.classList.remove("element-tap-focus");
  });
  container
    .querySelectorAll(`.element[data-element-number="${elementNumber}"]`)
    .forEach((el) => {
      el.classList.add("highlighted", "element-tap-focus");
    });
}

export function clearTableHighlights(container) {
...
export function highlightTableCategory(container, catClass) {
  highlightCategory(container, catClass);
}
...
let savedUnits = null;
try {
  const stored = localStorage.getItem('pt_units');
...
function saveUnits() {
  try {
    localStorage.setItem('pt_units', JSON.stringify(l3UnitState));
  } catch (e) {
...
function populateSimplifiedView(element) {
  ...
  if (greenCard) {
    ...
    if (window.appVersion === 'new' && v2Data) {
  ...
  if (yellowCard) {
    ...
    if (window.appVersion === 'new' && v2Data && v2Data.level2_atomic.mass.standard) {
    ...
    if (window.appVersion === 'new' && v2Data && v2Data.level3_properties?.electronic?.configuration) {
  ...
  if (redCard) {
    ...
    if (window.appVersion === 'new' && v2Data && v2Data.level4_history_stse.history.discoveryYear) {
...
export function showModal(element) {
  ...
  initElementTutorial();
  document.title = `Chemistry Practice - ${localizeElementName(element)}`;
  ...
  bindStepBar();
  initPracticeForElement(element.number);
}
...
export function initModalUI() {
  ...
  function closeElementModal() {
    ...
    document.title = "Chemistry Practice";
    clearHeadlineResizeHandler();
    cleanup3D(true);
    atomContainer.classList.remove("visible");
    resetModalUI();
